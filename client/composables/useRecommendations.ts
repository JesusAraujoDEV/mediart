...
export function useRecommendations(config = useRuntimeConfig()) {
    ...
    return tags
      .map((tag) => {
        return `${tag.title} (${tag.type})${
          tag.externalId ? ` - ID: ${tag.externalId}` : ""
  ...
  const sendData = async (
    ...
    try {
      ...
      if (selectedCategory.value === "mix" && Array.isArray(result?.mix)) {
        ...
        if (Array.isArray(result?.videogames))
          processed.push(...result.videogames);
      }
      ...
      recommendationsError.value =
        e?.message || "Ocurrió un error inesperado al obtener recomendaciones.";
      recommendations.value = [];
    } finally {
  ...
  const createPlaylist = async () => {
    if (!recommendations.value.length) {
      throw new Error(
        "No hay recomendaciones disponibles para crear una playlist."
      );
    }
    ...
    playlistCoverUrl =
      first?.coverUrl || first?.thumbnailUrl || first?.imageUrl || null;
    ...
    try {
      ...
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}));
        throw new Error(
          err.message || `Error al crear la playlist: ${resp.statusText}`
        );
      }
