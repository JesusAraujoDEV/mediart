    ...
    >
      ...
      <section class="w-2/3 glassEffect overflow-y-scroll h-full rounded-lg max-md:min-h-screen max-md:w-full p-6 custom-main-scroll">
        ...
        <div v-else-if="error" class="text-red-400 text-center flex flex-col items-center">
          ...
          <button @click="fetchFollowers" class="mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-full shadow-lg transition-colors text-lg">
            Reintentar
          </button>
        </div>
        ...
        <div v-else-if="filteredFollowers.length > 0" class="w-full flex flex-col gap-4 pb-4 px-2">
          ...
          >
            ...
            />
            <div class="flex-grow">
              <h3 class="text-xl font-bold text-white mb-1">{{ follower.username }}</h3>
              <p v-if="follower.bio" class="text-gray-300 text-sm mb-2">{{ follower.bio }}</p>
              <p class="text-xs text-gray-400">{{ follower.email }}</p>
            </div>
            <NuxtLink
...
// Computed property para filtrar los seguidores basado en la búsqueda
